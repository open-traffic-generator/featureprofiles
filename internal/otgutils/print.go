...
func LogLLDPMetrics(t testing.TB, otg *otg.OTG, c gosnappi.Config) {
	...
	for _, lldp := range c.Lldp().Items() {
		lldpMetric := otg.Telemetry().LldpInterface(lldp.Name()).Counters().Get(t)
...
func LogLLDPNeighborStates(t testing.TB, otg *otg.OTG, c gosnappi.Config) {
	...
	for _, lldp := range c.Lldp().Items() {
		lldpNeighborStates := otg.Telemetry().LldpInterface(lldp.Name()).LldpNeighborDatabase().LldpNeighborAny().Get(t)
