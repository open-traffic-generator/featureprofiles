...
func TestMplsStaticLspBGPNextHop(t *testing.T) {
	...
	t.Run("Verify IPv4 MPLS forwarding", func(t *testing.T) {
		validateTrafficFlows(t, ate, topo, ipv4Flow)
	})
	t.Run("Verify IPv6 MPLS forwarding", func(t *testing.T) {
		validateTrafficFlows(t, ate, topo, ipv6Flow)
	})
	t.Run("Verify IPv4 traffic discard when BGP-NH is not available.", func(t *testing.T) {
		withdrawBGPRoutes(t, []string{"BGP4.peer2.Route"})
		verifyIpv4TrafficDiscard(t, ate, topo)
	})
	t.Run("Verify IPv6 traffic discard when BGP-NH is not available.", func(t *testing.T) {
...
func configureATE(t *testing.T, ate *ondatra.ATEDevice) gosnappi.Config {
	...
	bgpPeerv4Routes.SetNextHopIpv4Address(atePort2.IPv4).
	...
	bgpPeerv6Routes.SetNextHopIpv6Address(atePort2.IPv6).
...
func verifyIpv4TrafficDiscard(t *testing.T, ate *ondatra.ATEDevice, config gosnappi.Config) {
	...
	for elapsed := 0; elapsed < 60; elapsed += 20 {
		...
		trafficState := verifyDiscardPkts(t, ate, ipv4Flow)
		if trafficState {
			t.Logf("Traffic Test Passed!")
		} else {
			t.Errorf("Traffic Test Failed!")
		}
		ate.OTG().StartTraffic(t)
	}
}
...
func verifyIpv6TrafficDiscard(t *testing.T, ate *ondatra.ATEDevice, config gosnappi.Config) {
	...
	for elapsed := 0; elapsed < 60; elapsed += 20 {
		...
		trafficState := verifyDiscardPkts(t, ate, ipv6Flow)
		if trafficState {
			t.Logf("Traffic Test Passed!")
		} else {
			t.Errorf("Traffic Test Failed!")
...
// Validate the Traffic Flows
func validateTrafficFlows(t *testing.T, ate *ondatra.ATEDevice, config gosnappi.Config, good string) {
	ate.OTG().StartTraffic(t)
	for elapsed := 0; elapsed < 60; elapsed += 20 {
		time.Sleep(trafficDuration * time.Second)
		ate.OTG().StopTraffic(t)
		otgutils.LogFlowMetrics(t, ate.OTG(), config)
		otgutils.LogPortMetrics(t, ate.OTG(), config)
		trafficState := verifyTrafficPkts(t, ate, good)
		if trafficState {
			t.Logf("Traffic Test Passed!")
		} else {
			t.Errorf("Traffic Test Failed!")
...
func verifyTrafficPkts(t *testing.T, ate *ondatra.ATEDevice, flowName string) bool {
	t.Helper()
	...
	if txPackets != 0 && rxPackets != 0 {
		lossPct = lostPackets * 100 / txPackets
		if txPackets == rxPackets && lossPct == 0 {
			return true
		} else {
			if lossPct > tolerancePkt {
				t.Errorf("Traffic Test Failed, got: %v, loss: %v want: 100%% ", rxPackets, lossPct)
				return false
			} else {
				return true
			}
...
// Verify the Traffic Discards
func verifyDiscardPkts(t *testing.T, ate *ondatra.ATEDevice, flowName string) bool {
	t.Helper()
	recvMetric := gnmi.Get(t, ate.OTG(), gnmi.OTG().Flow(flowName).State())
