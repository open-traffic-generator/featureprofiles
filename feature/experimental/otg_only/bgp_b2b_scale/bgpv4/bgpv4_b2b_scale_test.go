// Package otg_b2b_bgp_scale is an OTG-only (ATE port1 <-> ATE port2 back-to-back)
// BGPv4 session scale test written against the featureprofiles/ondatra framework.
//
// It uses only ondatra + gosnappi + OTG gNMI telemetry
// exactly like the reference b2b test in ../bgp_b2b/otgb2b_bgp_test.go.
//
// N BGPv4 sessions are brought up on each port (2*N peers total, each port1 peer
// paired with the port2 peer facing it), every peer advertises a /32 route range,
// and session state / route counters are read back over OTG gNMI. A capped number
// of data-plane flows over the advertised route ranges is used as a sanity check.
//
// Every device is double tagged (outer QinQ + inner dot1q) with a per-session tag
// pair,so each session pair sits in its own broadcast domain;
// the port L1 MTU is raised to carry those tags. All peers are iBGP in one AS
// Tagging (-scale_vlan / -scale_qinq) and the non-default BGP TCP ports
...
// ("<chassis>;<card>;<port>") to the real Ixia card and resource group behind
// them, and warns -- loudly, and again in the run summary -- when the requested
// scale is above what that card is rated for. The rating depends on the
// resource group layout, not just the card: an AresONE S400GD-16P-QDD group
// broken out 2x400G is rated for 18278 BGPv4 sessions per port, the same card
// broken out 16x50G for only 2285. Per-port memory / PCPU status and chassis
// CPU / memory are logged before each iteration and again at full scale. This
// needs the chassis credentials (-scale_chassis_user / -scale_chassis_pass,
// default admin/admin); pass -scale_chassis_check=false to skip discovery
// entirely.
//
// Every OTG API call is timed. One-shot calls (SetConfig, StartProtocols,
// StartTransmit, StopTransmit, StopProtocols) are highlighted with a
// ">>> OTG API" line as they happen, the GetMetrics fetches behind the poll
// loops are aggregated, and each iteration ends with a table of all of them.
//
// Refer featureprofiles/feature/experimental/otg_only/bgp_b2b_scale/README.md for more info
//
// to run the test use below cmd:
//
//	go test -v ./feature/experimental/otg_only/bgp_b2b_scale/bgpv4/bgpv4_b2b_scale_test.go -timeout 90m \
//	  -binding /home/azhar/fp/featuresprofile-ci/featureprofiles/feature/experimental/otg_only/otgb2b-hw.binding \
//	  -testbed /home/azhar/fp/featuresprofile-ci/featureprofiles/feature/experimental/otg_only/otgb2b-hw.testbed \
//	  -scale_sessions=100 -scale_routes=1 -scale_iterations=1 -scale_traffic=true -scale_max_flows=100 \
//	  -scale_session_timeout=10m | tee bgpv4_scale_100.log
//
...
var (
	...
	// AresONE S400GD-16P-QDD group split 2x400G is rated for 18278 BGPv4 sessions
	// per port, the same group split 16x50G for only 2285.
	chassisCheck   = flag.Bool("scale_chassis_check", false, "discover the Ixia card behind the bound ports and enforce its rated session/flow limits")
	...
	// Addressing. Every session pair gets its OWN /24, as in the Athena scale test:
	// session i uses subnetBase + i*256, with .1 on port1 and .2 on port2, and each
	// side uses the other as gateway / BGP peer.
	//
	// One subnet per session (and therefore per VLAN) is required, not cosmetic: on
	// this rig ixia-c answers ARP for a subnet from a single device, so sharing one
	// subnet across all devices leaves every device but the first without a
	// neighbor entry, and `start flows` then refuses to resolve the flow dst MAC.
...
func TestBGPv4B2BScale(t *testing.T) {
	...
	checkCardLimits(t, inv, sessions, 4)
	...
	for itr := uint(0); itr < *iterations; itr++ {
		...
		t.Logf("All %d BGPv4 sessions ESTABLISHED in %v", len(sc.peers), time.Since(upStart))
...
func buildScaleConfig(t *testing.T, sessions, routes uint32) *scaleConfig {
	...
	if *runTraffic && routes > 0 {
		...
		for i := uint32(1); i <= numFlows; i++ {
			...
			v4.Src().Increment().SetStart(routeStartFor(p1RouteStart, i, routes)).SetCount(routes)
			v4.Dst().Increment().SetStart(routeStartFor(p2RouteStart, i, routes)).SetCount(routes)
...
// configureSide adds one device (ethernet + vlans + ipv4 + BGPv4 peer + route
// range) for session index i on the given port, and returns the BGP peer name.
// host is the device's host octet inside the session subnet and gwHost is its
// gateway's (1 on port1, 2 on port2).
func configureSide(t *testing.T, config gosnappi.Config, portName, sidePfx, macStart, routeStart string, host, gwHost uint, i, routes, as uint32) string {
	t.Helper()
	...
	bgp := dev.Bgp().SetRouterId(devIP.String())
	...
	if routes > 0 {
		rr := peer.V4Routes().Add().
			...
			SetNextHopIpv4Address(devIP.String()).
...
// sessionIP returns host address `host` inside session i's own /24: session 1 is
// subnetBase+256 (18.0.1.0/24 by default), session 2 is 18.0.2.0/24, and so on.
// 18.0.0.0/8 covers 65535 sessions.
func sessionIP(t *testing.T, i uint32, host uint) net.IP {
	t.Helper()
	base := net.ParseIP(*subnetBase)
	if base == nil || base.To4() == nil {
		t.Fatalf("subnet base %q is not a valid IPv4 address", *subnetBase)
	}
	return nextIP(base, uint(i)*256+host)
}

// vlanIDs maps a 1-based session index onto its (inner dot1q, outer QinQ) tag pair,
// the inner id cycles through 1..4095 and the outer id advances once per full cycle,
// so both stay inside the 0..4095 range
...
func routeStartFor(base string, i, routes uint32) string {
	return nextIP(net.ParseIP(base), uint((i-1)*routes)).String()
}
...
func awaitSessionsEstablished(t *testing.T, otgDev *otg.OTG, peers []string, timeout time.Duration) {
	...
	for {
		...
		t.Logf("[t+%-6v] BGPv4 sessions ESTABLISHED: %d/%d | GetMetrics: state %v, counters %v | states: %s%s",
		...
		if time.Now().After(deadline) {
			t.Fatalf("timed out after %v: %d/%d BGPv4 sessions ESTABLISHED (states: %s)", timeout, st.up, want, st.histogram())
		}
...
func (c bgpCounterSum) String(wantPeers, wantRoutes uint64) string {
	...
	var b strings.Builder
	fmt.Fprintf(&b, "\n\t\tAggregated BGPv4 Metrics (OTG gNMI)\n%s\n", border)
...
// incrementMAC returns macStart incremented by (i-1).
...
// nextIP returns ip incremented by inc.
