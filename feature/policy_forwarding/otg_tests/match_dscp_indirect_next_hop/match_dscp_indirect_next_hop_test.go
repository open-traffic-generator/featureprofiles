...
func configureATE(t *testing.T) gosnappi.Config {
	...
	configureFlow(config, pfFlow, ap1.ID(),
		[]string{ap2.ID()},
		macAddress,
		ateP1.IPv4,
	...
	configureFlow(config, pfFlowv6, ap1.ID(),
		[]string{ap2.ID()},
		macAddress,
		ateP1.IPv6,
...
func configureBGPV6Dev(dev gosnappi.Device, Ipv6 gosnappi.DeviceIpv6, as int, bgpRoutev6 string) {
	...
	routes.SetNextHopIpv6Address(Ipv6.Address()).
...
func verifyPortTraffic(t *testing.T, ate *ondatra.ATEDevice, config gosnappi.Config, port1, port3 string) bool {
	startTime := time.Now()
	count := 0
	var got float64
	for time.Since(startTime) < timeout {
		...
		framesTx := gnmi.Get(t, ate.OTG(), gnmi.OTG().Port(port1).Counters().OutFrames().State())
		framesRx := gnmi.Get(t, ate.OTG(), gnmi.OTG().Port(port3).Counters().InFrames().State())
	...
	if count >= 2 {
		t.Logf("Packet loss percentage for port traffic: got %v, want %v", got, lossTolerance)
		return false
	}
	return true
}
